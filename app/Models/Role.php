...
class Role extends BaseModel implements RoleContract
{
    ...
    /**
     *  Scope filter isDepartment.
     */
    public function scopeIsDepartment($query)
    {
        return $query->where('is_department', (boolean) request()->query('is_department'));
    }
    ...
    {
        return $query->where("display_name", 'like', '%'.request()->query('display_name').'%');
    }

    /**
     *  Scope except role.
     */
    public function scopeExceptRole($query)
    {
        return $query->where("name", '!=', request()->query('except_role'));
    }
}
